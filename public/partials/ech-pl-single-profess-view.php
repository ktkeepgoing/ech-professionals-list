...
<div class="all_single_dr_wrap">
    <div class="sp_breadcrumb">
        <div><a href="<?= site_url() ?>"><?= $plugin_public->ECHPL_echolang(['Home', '主頁', '主页']) ?></a> > <a href="<?=site_url() . '/healthcare-professionals/' ?>"><?=$plugin_public->ECHPL_echolang(['Medical Professionals','醫護專業人員','医护专业人员'])?></a> > <?=$plugin_public->ECHPL_echolang([$json_arr['personnel']['en_salutation'].' '.$json_arr['personnel']['en_name'], $json_arr['personnel']['tc_name'].$json_arr['personnel']['tc_salutation'], $json_arr['personnel']['cn_name'].$json_arr['personnel']['cn_salutation']])?> </div>
    </div> <!-- sp_breadcrumb -->

    <div class="ECHDr_back_to_medical_list"><a href="<?=site_url()?>/healthcare-professionals/"> <?= $plugin_public->ECHPL_echolang(['Back to medical professionals', '返回醫護專業人員', '返回医护专业人员']) ?></a></div>
